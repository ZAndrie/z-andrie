...
export default function ExpertiseList({ items = [] }: { items?: any[] }) {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Delete "${title}" from GitHub Expertise-Repository?`)) return;
    setDeletingId(id);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-light mt-0.5">
              Synced with <strong className="text-[var(--color-primary)] font-medium">ZAndrie/Expertise-Repository</strong>
            </p>
          </div>
        </div>
        <a
